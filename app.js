...
function seleccionarMetodo(metodo) {
    ...
    if (metodo === 'efectivo') {
        // Abrir ventana nueva para cobro en efectivo
        const win = window.open(`efectivo.html?total=${encodeURIComponent(total)}`, '_blank', 'width=500,height=700');
        if (!win) {
            mostrarNotificacion('Permite ventanas emergentes para cobrar en efectivo', 'error');
        }
